...
## CORE METHODS ################################################################
    ...
    def train(self, X, Y, weights=None, minParent=2, maxDepth=np.inf, minLeaf=1, nFeatures=None):
        ...
        Args:
          ...
          weights (arr): M, or M,1 array of nonnegative data weights (will be normalized)
        ...
        self.__train_recursive(X, Y, weights, 0, minParent, maxDepth, minLeaf, nFeatures)
...
## HELPERS #####################################################################
    ...
    def __train_recursive(self, X, Y, W, depth, minParent, maxDepth, minLeaf, nFeatures):
        ...
        for i_feat in try_feat[0:nFeatures]:
            ...
            wsorted = W[pi]                                              # ...and sort targets by feature ID
            ...
            val,idx = self.data_impurity(tsorted, wsorted, can_split)
        ...
        self.__train_recursive(X[go_left,:], Y[go_left], W[go_left], depth+1, minParent, maxDepth, minLeaf, nFeatures)

        # recur right
        self.R[my_idx] = self.sz    
        self.__train_recursive(X[go_right,:], Y[go_right], W[go_right], depth+1, minParent, maxDepth, minLeaf, nFeatures)
...
class treeRegress(treeBase,regressor):
    ...
    @staticmethod
    def min_weighted_var(tsorted, wsorted, can_split):   # TODO!!! ADD WEIGHTS
        ...
        n = tsorted.shape[0]
        weight_to = np.cumsum(wsorted, axis=0);
        weight_pa = weight_to[-1] - weight_to;
        weight_pa /= weight_to[-1]; weight_to /= weight_to[-1]; weight_pa[-1]=1.;
        if len(tsorted.shape)>1:
            weight_to, weight_pa = weight_to.reshape(-1,1), weight_pa.reshape(-1,1)
        y_cum_to = np.cumsum(wsorted * tsorted, axis=0)
        y_cum_pa = y_cum_to[-1] - y_cum_to
        mean_to = y_cum_to / weight_to; 
        mean_pa = y_cum_pa / weight_pa; 
        ...
        var_to = (y2_cum_to - 2 * mean_to * y_cum_to + np.power(mean_to, 2)*weight_to) / weight_to
        var_pa = (y2_cum_pa - 2 * mean_pa * y_cum_pa + np.power(mean_pa, 2)*weight_pa) / weight_pa
        ...
        weighted_variance = weight_to/weight_to[-1] * var_to + weight_pa/weight_to[-1] * var_pa
...
class treeClassify(treeBase,classifier):
    ...
    @staticmethod
    def entropy(tsorted, wsorted, can_split):
        """Return the value and index of the minimum of the Shannon entropy impurity score"""
        n = tsorted.shape[0]
        eps = np.spacing(1)
        y_left = np.cumsum(tsorted*wsorted.reshape(-1,1), axis=0)
        y_right = y_left[-1,:] - y_left         # construct p(class) for each possible split
        wts_left = np.cumsum(wsorted)           # by counting & then normalizing by left/right sizes
        y_left /= wts_left.reshape(-1,1)
        tmp = wts_left[-1] - wts_left; tmp[-1] = 1;
...
#        ##y_left = np.cumsum(tsorted*wsorted, axis=0)
#        y_right = y_left[-1,:] - y_left         # construct p(class) for each possible split
#        wts_left = np.arange(1.0,n+1)     # by counting & then normalizing by left/right sizes
#        ##wts_left = np.cumsum(wsorted[:,0])  # TODO by counting & then normalizing by left/right sizes
#        y_left /= wts_left.reshape(-1,1)
#        tmp = n - wts_left
#        tmp[-1] = 1
#        y_right /= tmp.reshape(-1,1)
